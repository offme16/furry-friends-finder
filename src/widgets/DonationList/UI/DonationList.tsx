import {
  ...
  getDonationsData,
  offsetDonationsPage,
  DonationsFeedSliceActions,
} from "../../../enteties/DonationFeed";
...
const DonationList = () => {
  ...
  const placeholders = Array(6).fill(
    ...
    0,
    offset.limit - Number(donations?.length)
  );
  const donationItem = (
    <div
      className={`${cls.donation_card_container} ${
        offset.isLoading || offset.error ? cls.loading : null
      }`}
    >
      <span
        className={cls.pagination_left}
        onClick={() => handleSetPage(offset.page - 1)}
      >
      ...
      {placeholders}
      <span
        className={cls.pagination_right}
        onClick={() => handleSetPage(offset.page + 1)}
      >
  ...
  return (
    <div className={cls.DonationList}>
      <SectionHeader
        mainText={"Ты можешь помочь нам!"}
        subText={"Здесь ты можешь помочь нам и нашим пушистым друзьям!"}
      ></SectionHeader>
